        ...
        try:
            with open(r"transactions.json", "r", encoding="UTF-8") as file:
                ...
                for transaction in transactions_data:
                    ...
                    elif transaction_type == "OutgoingPaymentDomestic":
                        OutgoingPaymentDomestic(**transaction)
...
@dataclass
class OutgoingPaymentDomestic(Transaction):
    """Data structure representing domestic outgoing payments."""

    text_identifiers: ClassVar[dict] = {"csob": ["Odchozí úhrada"], "cs": ["Tuzemská odchozí úhrada"]}
    """Class attribute to identify domestic outgoing payments section
        in text."""
    all: ClassVar[List] = []
    """Class attribute to hold all created instances of domestic
        outgoing payments."""

    def __post_init__(self):
        """Post initialize method to track each created instace."""
        super().__post_init__()
        OutgoingPaymentDomestic.all.append(self)

    def __str__(self) -> str:
        return (
            f"Tuzemská odchozí úhrada\n"
            "\n"
            f"ID tx:            {self.transaction_id}\n"
            f"účet transakce:   {self.statement_account}\n"
            f"na účet:          {self.account_to}\n"
    ...
    def create(
        ...
        line_index: int,
        extracted_text_lines: list[str],
    ) -> "OutgoingPaymentDomestic":
        """Create and return new instance of OutgoingPaymentDomestic.
        ...
        Args:
            ...
            account_from (str): Account number associated with given
                pdf statement.
            line_index (int): Index of line with domestic outgoing payment
                transaction text.
            extracted_text_lines (list[str]): All the text lines from
                extracted pdf account statement.

        Returns:
            OutgoingPaymentDomestic: Instance representing specific
        ...
        if bank == "cs":
            ...
            amount_line = get_amount_line(extracted_text_lines, line_index)[0]
            amount = pdf_amount_to_float(amount_line.strip())
            return OutgoingPaymentDomestic(
                statement_account=statement_account,
                year=year,
                account_from=account_from,
                amount=amount,
...
@dataclass
class OutgoingPaymentPeriodic(OutgoingPaymentDomestic):
    """Data structure representing periodic domestic outgoing payments."""

    text_identifiers: ClassVar[dict] = {"cs": ["Trvalý příkaz"], "csob": ["Trvalý příkaz"]}
    """Class attribute to identify periodic domestic outgoing payments section
        in text."""
    all: ClassVar[List] = []
    """Class attribute to hold all created instances of periodic domestic
        outgoing payments."""
...
@dataclass
class CardPaymentDebit(OutgoingPaymentDomestic):
...
class CardPaymentIncoming(CardPaymentDebit):
    ...
    text_identifiers: ClassVar[dict] = {"csob": ["Příchozí úhrada kartou"], "cs": ["PLACEHOLDER"]}
...
@dataclass
class TaxInterest(OutgoingPaymentDomestic):
...
@dataclass
class DirectDebit(OutgoingPaymentDomestic):
